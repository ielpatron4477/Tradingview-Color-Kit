...
function LiveChart({th,fam,candleStyle='candles'}){
  ...
  const[live,setLive]=useState(()=>{const p=100;return{o:p,h:p+1,l:p-1,c:p};});
  ...
  useEffect(()=>{
    const id=setInterval(()=>{
      ...
      setLive(prev=>{
        ...
        const nc=Math.max(prev.o-7,Math.min(prev.o+7,prev.c+noise+drift));
        const nh=Math.max(prev.h,nc+Math.random()*0.25);
        const nl=Math.min(prev.l,nc-Math.random()*0.25);
        if(tickRef.current%70===0){
          setCandles(d=>[...d.slice(-27),prev]);
          return{o:prev.c,h:prev.c,l:prev.c,c:prev.c};
        }
        return{...prev,c:nc,h:nh,l:nl};
      });
    },50);
  ...
  const prices=drawAll.flatMap(cv=>[cv.h,cv.l]);
