...
const getContrast=(h1,h2)=>{const l1=getLum(h1),l2=getLum(h2),hi=Math.max(l1,l2),lo=Math.min(l1,l2);return((hi+0.05)/(lo+0.05));};
...
const getSimilar=(th,palette,n=4)=>{const fam=th[palette];return THEMES.filter(t=>t.id!==th.id).map(t=>{const f=t[palette];return{t,d:colorDist(fam.opens,f.opens)+colorDist(fam.highs,f.highs)+colorDist(fam.lows,f.lows)+colorDist(fam.mids,f.mids)};}).sort((a,b)=>a.d-b.d).slice(0,n).map(x=>x.t);};

// ─── LIVE ANIMATED CHART ─────────────────────────────────────────────────────
function LiveChart({th,fam,siteMode}){
  const canvasRef=useRef(null);
  ...
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      allCandles.forEach((c,i)=>{
        const x=i*cw+1;const isLive=i===allCandles.length-1;
        const bull=c.c>=c.o;
  ...
  },[th.id,palette,siteMode]);

  const palette=fam===th.match?'match':'vivid'; // approximation for dep
  return(
    ...
    <div style={{minHeight:"100vh",background:site.bg,position:"relative",transition:"background 0.3s"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div className="main-pad" style={{padding:"72px 52px 48px",maxWidth:1500,margin:"0 auto"}}>
              ...
              <div key={th.id} style={{borderRadius:24,...glassStyle,overflow:"hidden",animation:"slideIn 0.3s ease",boxShadow:glow?`0 24px 80px rgba(0,0,0,0.6), 0 0 60px ${th.ac}15`:"0 24px 60px rgba(0,0,0,0.5)"}}>
                ...
                <div style={{padding:"32px 28px 24px"}}>
                  ...
                  <div style={{marginBottom:16,position:"relative"}}>
                    <LiveChart th={th} fam={fam} siteMode={siteMode}/>
                    <button onClick={()=>setShowFullChart(true)} style={{position:"absolute",bottom:26,left:"50%",transform:"translateX(-50%)",padding:"5px 14px",borderRadius:16,background:"rgba(0,0,0,0.5)",border:"1px solid rgba(255,255,255,0.15)",color:"rgba(255,255,255,0.7)",fontSize:9,fontFamily:"'JetBrains Mono',monospace",cursor:"pointer",letterSpacing:"0.1em",backdropFilter:"blur(8px)"}}>
                      ⤢ FULLSCREEN
                    </button>
                  </div>
          ...
          {/* KITS LIKE THIS */}
          {(()=>{const similar=getSimilar(th,palette,4);return(<div style={{marginBottom:48}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10}}>
              {similar.map(t=>{const f=t[palette];const tg=["NEON","VIBE","DEGEN","CYBER","FREAK","AURA","GUM","FREE","ANIME","MEME"].includes(t.cat);const bg=SBGS[t.id]||PHANTOM_SBGS[t.id]||t.bg;const r=getContrast(t.up,t.bg);const b=contrastBadge(r);return(
                <button key={t.id} onClick={()=>setTheme(t.id)} className="kit-card" style={{padding:"14px 12px",borderRadius:12,background:bg,border:"1px solid rgba(255,255,255,0.06)",cursor:"pointer",textAlign:"left",backdropFilter:"blur(8px)",boxShadow:"0 2px 8px rgba(0,0,0,0.4)"}}>
                  <div style={{display:"flex",gap:4,marginBottom:8}}>
                    ...
                    <div style={{width:15,height:15,borderRadius:4,background:t.dn,border:"1px solid rgba(255,255,255,0.08)"}}/>
                  </div>
                  <div style={{fontSize:10,fontFamily:"'Syne',sans-serif",color:t.tx,fontWeight:700,marginBottom:4}}>{t.name}</div>
                  <div style={{display:"flex",gap:4,alignItems:"center"}}>
                    {[f.opens,f.highs,f.lows,f.mids].map((col,i)=><div key={i} style={{width:8,height:8,borderRadius:"50%",background:col,boxShadow:`0 0 6px ${col}`}}/>)}
