  ...
  const KitPanel=({th,fam,glow,label})=>(
    <div style={{flex:1,borderRadius:16,overflow:"hidden",background:SBGS[th.id]||PHANTOM_SBGS[th.id]||th.bg,border:`1px solid ${th.ac}55`,boxShadow:`0 8px 32px ${th.ac}20`}}>
      <div style={{padding:"20px 18px"}}>
        ...
        <div style={{fontSize:28,fontWeight:800,fontFamily:"'Syne',sans-serif",color:"#f0f0f0",lineHeight:1,marginBottom:6}}>{th.name}</div>
    ...
    <div style={{position:"fixed",inset:0,zIndex:999,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",padding:24}} onClick={onClose}>
      <div style={{background:"#0a0a12",borderRadius:24,padding:28,border:"1px solid rgba(255,255,255,0.08)",maxWidth:800,width:"100%",animation:"fadeUp 0.3s ease"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <span style={{fontSize:13,fontFamily:"'Syne',sans-serif",fontWeight:700,color:"#555",letterSpacing:"0.1em"}}>KIT COMPARE</span>
        ...
        <div style={{display:"flex",gap:14}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"0 8px",color:"#222",fontSize:20,fontWeight:800,fontFamily:"'Syne',sans-serif",flexShrink:0}}>VS</div>
    ...
    <div style={{position:"fixed",inset:0,zIndex:999,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",padding:24}} onClick={onClose}>
      <div style={{background:"#0a0a12",borderRadius:20,padding:28,border:"1px solid rgba(255,255,255,0.08)",maxWidth:380,width:"100%",animation:"fadeUp 0.3s ease"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:20}}>
          <span style={{fontSize:13,fontFamily:"'Syne',sans-serif",fontWeight:700,color:"#555",letterSpacing:"0.1em"}}>⌨ KEYBOARD SHORTCUTS</span>
        ...
        {shortcuts.map(([key,desc])=>(
          <div key={key} style={{display:"flex",alignItems:"center",gap:14,padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
            ...
            <span style={{fontSize:13,fontFamily:"'Syne',sans-serif",color:"#444"}}>{desc}</span>
          </div>
        ))}
      </div>
    </div>
...
// ─── GALLERY MODAL ────────────────────────────────────────────────────────────
function GalleryModal({th,gallery,onSubmit,onClose}){
  const[url,setUrl]=useState("");
  const[name,setName]=useState("");
  const kitGallery=gallery.filter(g=>g.kitId===th.id);
  const submit=()=>{if(url.trim()){onSubmit(url.trim(),name.trim()||"anon",th.id);setUrl("");setName("");}};
  return(
    <div style={{position:"fixed",inset:0,zIndex:999,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"center",justifyContent:"center",padding:24}} onClick={onClose}>
      <div style={{background:"#0a0a12",borderRadius:24,padding:28,border:"1px solid rgba(255,255,255,0.08)",maxWidth:700,width:"100%",maxHeight:"85vh",overflowY:"auto",animation:"fadeUp 0.3s ease"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
          <span style={{fontSize:13,fontFamily:"'Syne',sans-serif",fontWeight:700,color:"#555",letterSpacing:"0.1em"}}>📸 CHARTS MADE WITH {th.name.toUpperCase()}</span>
          <button onClick={onClose} style={{background:"none",border:"none",color:"#333",fontSize:20,cursor:"pointer"}}>✕</button>
        </div>
        <div style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color:"#222",marginBottom:20,letterSpacing:"0.08em"}}>Share your chart screenshot with this kit</div>
        <div style={{display:"flex",gap:8,marginBottom:24,flexWrap:"wrap"}}>
          <input value={url} onChange={e=>setUrl(e.target.value)} placeholder="Paste image URL (Twitter, Imgur, etc.)..." style={{flex:2,minWidth:200,padding:"10px 14px",borderRadius:10,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",color:"#888",fontSize:12,fontFamily:"'JetBrains Mono',monospace",outline:"none"}}/>
          <input value={name} onChange={e=>setName(e.target.value)} placeholder="Your CT name (optional)" style={{flex:1,minWidth:140,padding:"10px 14px",borderRadius:10,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",color:"#888",fontSize:12,fontFamily:"'JetBrains Mono',monospace",outline:"none"}}/>
          <button onClick={submit} style={{padding:"10px 20px",borderRadius:10,background:th.ac,color:"#000",border:"none",fontSize:11,fontFamily:"'Syne',sans-serif",fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>Submit ↑</button>
        </div>
        {kitGallery.length===0?(
          <div style={{textAlign:"center",padding:"40px 0",color:"#1e1e1e",fontFamily:"'JetBrains Mono',monospace",fontSize:12}}>No screenshots yet — be the first to submit one!</div>
        ):(
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
            {kitGallery.map(g=>(
              <div key={g.id} style={{borderRadius:10,overflow:"hidden",border:"1px solid rgba(255,255,255,0.06)"}}>
                <img src={g.url} alt="chart" style={{width:"100%",height:130,objectFit:"cover",display:"block"}} onError={e=>e.target.style.display="none"}/>
                <div style={{padding:"8px 10px",display:"flex",justifyContent:"space-between"}}>
                  <span style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color:"#333"}}>{g.name||"anon"}</span>
                  <span style={{fontSize:9,fontFamily:"'JetBrains Mono',monospace",color:"#1e1e1e"}}>{new Date(g.date).toLocaleDateString()}</span>
                </div>
              </div>
            ))}
          </div>
...
export default function App(){
  ...
  const[gallery,setGallery]=useState(()=>{try{return JSON.parse(localStorage.getItem('tck-gallery')||'[]')}catch{return[]}});
  ...
  // Gallery submit
  const submitGallery=(url,name,kitId)=>{
    const entry={url,name,kitId,date:Date.now(),id:Math.random().toString(36).slice(2)};
    setGallery(prev=>{const next=[entry,...prev].slice(0,100);try{localStorage.setItem('tck-gallery',JSON.stringify(next));}catch{}return next;});
    showToast('📸 Screenshot submitted!');
  };
  ...
  useEffect(()=>{
    const h=(e)=>{
      ...
      else if(e.key==='Escape'){setShowCompare(false);setShowGallery(false);setShowKB(false);setSearch('');}
  ...
  const exportPine=()=>{
    ...
    const k=T2.kl; const ic=T2.ict; const v=T2.vwap;
    const hex=(h)=>h.slice(0,7);
    const pine=`//@version=5
// ============================================================
// ${th.name.toUpperCase()} Color Kit — generated by trading-color-kit.vercel.app
// by aSian (@_a_Sian_) · Palette: ${palette.toUpperCase()}
// ============================================================
indicator("${th.name} Kit — Key Levels", overlay=true, max_lines_count=500)

// ── COLOR FAMILIES ───────────────────────────────────────────
opens_color  = color.new(color.rgb(${parseInt(hex(fam.opens).slice(1,3),16)},${parseInt(hex(fam.opens).slice(3,5),16)},${parseInt(hex(fam.opens).slice(5,7),16)}), 0)
highs_color  = color.new(color.rgb(${parseInt(hex(fam.highs).slice(1,3),16)},${parseInt(hex(fam.highs).slice(3,5),16)},${parseInt(hex(fam.highs).slice(5,7),16)}), 0)
lows_color   = color.new(color.rgb(${parseInt(hex(fam.lows).slice(1,3),16)},${parseInt(hex(fam.lows).slice(3,5),16)},${parseInt(hex(fam.lows).slice(5,7),16)}), 0)
mids_color   = color.new(color.rgb(${parseInt(hex(fam.mids).slice(1,3),16)},${parseInt(hex(fam.mids).slice(3,5),16)},${parseInt(hex(fam.mids).slice(5,7),16)}), 0)

// ── INPUTS ───────────────────────────────────────────────────
show_opens   = input.bool(true,  "Show Opens")
show_highs   = input.bool(true,  "Show Prev Highs")
show_lows    = input.bool(true,  "Show Prev Lows")
show_mids    = input.bool(false, "Show Midpoints")
show_npoc    = input.bool(true,  "Show nPOC Levels")
show_sr      = input.bool(true,  "Show S/R Zones")
show_vwap    = input.bool(true,  "Show VWAP Bands")
show_ict     = input.bool(false, "Show ICT Levels")
...
if dayofweek == dayofweek.monday and timeframe.isintraday
...
if show_opens
    hline(d_open,  "Daily Open",   color=${hex(k.dOpen)},   linestyle=hline.style_solid,  linewidth=1)
    hline(w_open,  "Weekly Open",  color=${hex(k.wOpen)},   linestyle=hline.style_dashed, linewidth=1)
    hline(m_open,  "Monthly Open", color=${hex(k.mOpen)},   linestyle=hline.style_dashed, linewidth=2)

// ── VWAP BANDS — ${hex(fam.opens)} ──────────────────────────
[vwap_val, upper1, lower1] = ta.vwap(hlc3, false, 1)
upper2 = vwap_val + 2 * (upper1 - vwap_val)
lower2 = vwap_val - 2 * (vwap_val - lower1)
upper3 = vwap_val + 3 * (upper1 - vwap_val)
lower3 = vwap_val - 3 * (vwap_val - lower1)

if show_vwap
    plot(vwap_val, "VWAP",   color=${hex(v.vwap)},    linewidth=2)
    plot(upper1,   "+1σ",    color=${hex(v.band1pos)}, linewidth=1)
    plot(lower1,   "-1σ",    color=${hex(v.band1neg)}, linewidth=1)
    plot(upper2,   "+2σ",    color=${hex(v.band2pos)}, linewidth=1, style=plot.style_linebr)
    plot(lower2,   "-2σ",    color=${hex(v.band2neg)}, linewidth=1, style=plot.style_linebr)
    plot(upper3,   "+3σ",    color=${hex(v.band3pos)}, linewidth=1, style=plot.style_circles)
    plot(lower3,   "-3σ",    color=${hex(v.band3neg)}, linewidth=1, style=plot.style_circles)

// ── CHART STYLE REFERENCE ───────────────────────────────────
// Paste these into TradingView Chart Settings manually:
//
//   Background:    ${hex(th.bg)}
//   Up Candle:     ${hex(th.up)}
//   Down Candle:   ${hex(th.dn)}
//   Wick Color:    ${hex(th.wk)}
//   Text Color:    ${hex(th.tx)}
//   Grid Lines:    ${hex(th.gr)}
//
// ── TOOL COLOR REFERENCE ─────────────────────────────────────
// Key Level Colors (apply to your indicators manually):
//   Opens:            ${hex(fam.opens)}
//   Highs / Resist:   ${hex(fam.highs)}
//   Lows / Support:   ${hex(fam.lows)}
//   Mids / Neutral:   ${hex(fam.mids)}
//   PDH:              ${hex(k.pdh)}
//   PDL:              ${hex(k.pdl)}
//   PWH:              ${hex(k.pwh)}
//   PWL:              ${hex(k.pwl)}
//   PMH:              ${hex(k.pmh)}
//   PML:              ${hex(k.pml)}
//   Daily nPOC:       ${hex(k.dNPOC)}
//   Weekly nPOC:      ${hex(k.wNPOC)}
//   Monthly nPOC:     ${hex(k.mNPOC)}
//   Daily S/R Hi:     ${hex(k.srDayH)}
//   Daily S/R Lo:     ${hex(k.srDayL)}
//   Weekly S/R Hi:    ${hex(k.srWkH)}
//   Weekly S/R Lo:    ${hex(k.srWkL)}
//   BOS:              ${hex(ic.bos)}
//   ChoCH:            ${hex(ic.choch)}
//   OTE Zone:         ${hex(ic.ote)}
//   IFVG:             ${hex(ic.ifvg)}
//   London KZ:        ${hex(ic.kzLon)}
//   NY KZ:            ${hex(ic.kzNY)}
// ============================================================
`;
    ...
    a.download=`${th.id}-${palette}-kit.pine`;a.click();
    showToast('🌲 Pine Script downloaded — paste into TV editor!');
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:"#05050f",position:"relative"}}>
        ...
        <div className="main-pad" style={{padding:"72px 52px 48px",maxWidth:1500,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
              {(()=>{const kt=THEMES.find(t=>t.id===kotd);if(!kt)return null;return(
                <button onClick={()=>setTheme(kotd)} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderRadius:12,background:"rgba(255,215,0,0.06)",border:"1px solid rgba(255,215,0,0.18)",cursor:"pointer"}}>
                  ...
                  <div><div style={{fontSize:7,fontFamily:"'JetBrains Mono',monospace",color:"#665500",letterSpacing:"0.2em"}}>KIT OF THE DAY</div><div style={{fontSize:10,fontFamily:"'Syne',sans-serif",fontWeight:700,color:"#aa8800"}}>{kt.name}</div></div>
                </button>
              );})()}
              {(()=>{const st=THEMES.find(t=>t.id===seasonal.id);if(!st)return null;return(
                <button onClick={()=>setTheme(seasonal.id)} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderRadius:12,background:`${seasonal.color}0c`,border:`1px solid ${seasonal.color}28`,cursor:"pointer"}}>
                  <span style={{fontSize:13}}>🌟</span>
                  <div><div style={{fontSize:7,fontFamily:"'JetBrains Mono',monospace",color:seasonal.color,letterSpacing:"0.18em",opacity:0.7}}>SEASONAL</div><div style={{fontSize:10,fontFamily:"'Syne',sans-serif",fontWeight:700,color:seasonal.color}}>{seasonal.label}</div></div>
                </button>
              );})()}
                ...
                <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                  {recent.filter(id=>id!==activeTheme).slice(0,5).map(id=>{const t=THEMES.find(x=>x.id===id);if(!t)return null;return(
                    <button key={id} onClick={()=>setTheme(id)} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 9px",borderRadius:16,background:SBGS[id]||t.bg,border:`1px solid ${t.ac}40`,cursor:"pointer"}}>
                      ...
                      <span style={{fontSize:8,fontFamily:"'Syne',sans-serif",color:t.tx,fontWeight:700}}>{t.name}</span>
                    </button>
                  );})}
                </div>
              </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:9,marginBottom:64}}>
            ...
            {displayed.map(t=>{
              ...
              return(
                <button key={t.id} onClick={()=>{if(compareMode&&t.id!==activeTheme){setCompareId(t.id);setShowCompare(true);}else{setTheme(t.id);}}} className={`kit-card${active?" active":""}${compareMode?" compare-hover":""}`}
                  ...
                  {isTrending&&!['🏆','🌟'].length&&<div style={{position:"absolute",top:8,right:8,fontSize:8}}>🔥</div>}
              ...
              <div key={th.id} style={{borderRadius:24,...glassStyle,overflow:"hidden",animation:"slideIn 0.3s ease",boxShadow:glow?`0 24px 80px rgba(0,0,0,0.6), 0 0 60px ${th.ac}15`:"0 24px 60px rgba(0,0,0,0.5)"}}>
                ...
                <div style={{padding:"32px 28px 24px"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    ...
                    <button onClick={copyAll} style={{padding:"10px",borderRadius:10,...glassStyle,color:"#555",border:"1px solid rgba(255,255,255,0.08)",fontSize:10,fontFamily:"'Syne',sans-serif",fontWeight:700,cursor:"pointer",letterSpacing:"0.06em"}}>
                      📋 COPY ALL
                    </button>
                    <button onClick={exportPine} style={{padding:"10px",borderRadius:10,background:"rgba(0,180,100,0.12)",color:"#20c870",border:"1px solid rgba(0,180,100,0.25)",fontSize:10,fontFamily:"'Syne',sans-serif",fontWeight:700,cursor:"pointer",letterSpacing:"0.06em"}}
                      onMouseEnter={e=>e.currentTarget.style.background="rgba(0,180,100,0.2)"}
                      onMouseLeave={e=>e.currentTarget.style.background="rgba(0,180,100,0.12)"}>
                      🌲 PINE SCRIPT
                    </button>
                  </div>
                  ...
                  <div style={{display:"flex",gap:8,marginTop:8}}>
                    <button onClick={()=>setShowGallery(true)} style={{flex:1,padding:"9px",borderRadius:10,background:"rgba(255,255,255,0.03)",color:"#444",border:"1px solid rgba(255,255,255,0.07)",fontSize:10,fontFamily:"'Syne',sans-serif",fontWeight:600,cursor:"pointer",letterSpacing:"0.05em"}}>
                      📸 GALLERY ({gallery.filter(g=>g.kitId===th.id).length})
                    </button>
                    <button onClick={()=>{setCompareMode(v=>!v);if(!compareMode)showToast('Click any kit card to compare');}} style={{flex:1,padding:"9px",borderRadius:10,background:compareMode?"rgba(255,200,0,0.12)":"rgba(255,255,255,0.03)",color:compareMode?"#aa8800":"#444",border:"1px solid rgba(255,255,255,0.07)",fontSize:10,fontFamily:"'Syne',sans-serif",fontWeight:600,cursor:"pointer",letterSpacing:"0.05em"}}>
      ...
      {showGallery&&<GalleryModal th={th} gallery={gallery} onSubmit={submitGallery} onClose={()=>setShowGallery(false)}/>}
