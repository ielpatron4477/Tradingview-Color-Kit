...
export default function App(){
  ...
  const[gridExpanded,setGridExpanded]=useState(()=>window.innerWidth>900);
    ...
    <div style={{minHeight:"100vh",background:site.bg,position:"relative",transition:"background 0.3s"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div className="main-pad" style={{padding:"72px 52px 48px",maxWidth:1500,margin:"0 auto"}}>
          ...
          {/* THEME GRID */}
          <div className="kit-grid" style={{display:gridCollapsed?"none":"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:9,marginBottom:gridCollapsed?0:64}} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
            ...
            {displayed.map(t=>{
              ...
              return(
                <button key={t.id} onClick={()=>{if(compareMode&&t.id!==activeTheme){setCompareId(t.id);setShowCompare(true);}else{setTheme(t.id);if(window.innerWidth<=900)setGridCollapsed(true);}}} onMouseMove={e=>{if(window.innerWidth>900){const r=e.currentTarget.getBoundingClientRect();const x=((e.clientX-r.left)/r.width-0.5)*18;const y=((e.clientY-r.top)/r.height-0.5)*18;e.currentTarget.style.transform="perspective(500px) rotateY("+x+"deg) rotateX("+(-y)+"deg) scale(1.04)";}}} onMouseLeave={e=>{e.currentTarget.style.transform="";hideTooltip();}} onMouseEnter={e=>{if(window.innerWidth>900)showTooltip(t,e);}} className={"kit-card"+(active?" active":"")+(compareMode?" compare-hover":"")}
          ...
          {/* On mobile, after selecting a kit, collapse grid */}
