  ...
  return(
    <div onClick={handleCopy}
      ...
      <div className={pop?"copy-pop":""} style={{width:"100%",height:36,borderRadius:10,background:hex,
        ...
        border:`2px solid ${cp?"rgba(255,255,255,0.6)":"rgba(255,255,255,0.08)"}`,
          ...
          return(
            <div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 8px",borderRadius:8,background:`${b.color}0c`,border:`1px solid ${b.color}28`}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:8,color:"#444",fontFamily:"'JetBrains Mono',monospace",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{p.la} / {p.lb}</div>
              </div>
              <div style={{flexShrink:0,padding:"2px 5px",borderRadius:4,background:`${b.color}20`,fontSize:7,fontFamily:"'JetBrains Mono',monospace",color:b.color,fontWeight:700}}>{b.label} {r.toFixed(1)}</div>
            </div>
          );
        })}
      </div>
    ...
    <div onClick={onExit} style={{position:"fixed",inset:0,zIndex:9999,background:bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"background 1.5s ease"}}>
      ...
      <div style={{width:s,height:s,perspective:900,perspectiveOrigin:"50% 55%",marginBottom:40}}>
        <div style={{width:s,height:s,position:"relative",transformStyle:"preserve-3d",animation:"ss-spin 6s linear infinite"}}>
          {faces.map((f,i)=>(<div key={i} style={{position:"absolute",width:s,height:s,background:f.bg,transform:f.t,border:"1px solid rgba(255,255,255,0.12)",backfaceVisibility:"hidden",opacity:f.o}}/>))}
        </div>
      </div>
      <div style={{fontSize:14,fontFamily:"'Syne',sans-serif",fontWeight:800,color:th.ac,letterSpacing:"0.2em",marginBottom:6,textShadow:`0 0 30px ${th.ac}80`,transition:"color 1.5s ease"}}>{th.name}</div>
...
export default function App(){
  ...
  useEffect(()=>{
    const h=(e)=>{
      ...
      else if(e.key==='Escape'){setShowCompare(false);setShowGallery(false);setShowKB(false);setSearch('');setShowScreensaver(false);}
    ...
    <div style={{minHeight:"100vh",background:site.bg,position:"relative",transition:"background 0.3s"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div className="main-pad" style={{padding:"72px 52px 48px",maxWidth:1500,margin:"0 auto"}}>
          ...
          {/* THEME GRID */}
          <div className="kit-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:9,marginBottom:64}}>
            ...
            {displayed.map(t=>{
              ...
              return(
                <button key={t.id} onClick={()=>{if(compareMode&&t.id!==activeTheme){setCompareId(t.id);setShowCompare(true);}else{setTheme(t.id);}}} className={`kit-card${active?" active":""}${compareMode?" compare-hover":""}`}
                  ...
                  <div style={{fontSize:10,fontFamily:"'Syne',sans-serif",color:active?t.tx:"rgba(255,255,255,0.5)",fontWeight:700,letterSpacing:"0.04em",marginBottom:3}}><Highlight text={t.name} query={search} color={t.ac}/></div>
          ...
          {/* MAIN DETAIL */}
          <div className="detail-grid" style={{display:"grid",gridTemplateColumns:"380px 1fr",gap:40,alignItems:"start"}} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>

            {/* LEFT CARD */}
            <div className="sticky-card" style={{position:"sticky",top:80}}>
              <div key={th.id} className="kit-slide glass-shimmer" style={{borderRadius:24,...glassStyle,overflow:"hidden",boxShadow:glow?`0 24px 80px rgba(0,0,0,0.6), 0 0 60px ${th.ac}15`:"0 24px 60px rgba(0,0,0,0.5)"}}>
            ...
            <div>
              ...
              <div className="glass-shimmer" style={{...glassStyle,borderRadius:20,overflow:"hidden"}}>
