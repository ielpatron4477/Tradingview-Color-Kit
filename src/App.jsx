...
function ColorRow({hex,label,sub,isLine=true}){
  ...
  return(
    <div onClick={()=>{navigator.clipboard?.writeText(b);sc(true);setTimeout(()=>sc(false),1400);}}
      ...
      <div style={{flex:1}}>
        <div style={{fontSize:14,color:"#c8c0b8",fontWeight:500,fontFamily:"'Space Grotesk',sans-serif"}}>{label}</div>
...
export default function App(){
  ...
  useEffect(()=>{ window.__kitAccent=th.ac; },[th.ac]);
        ...
        <div className="main-pad" style={{padding:"72px 52px 48px",maxWidth:1500,margin:"0 auto"}}>
          ...
          <div className="kit-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:9,marginBottom:64}}>
            ...
            {displayed.map(t=>{
              ...
              return(
                <button key={t.id} onClick={()=>{if(compareMode&&t.id!==activeTheme){setCompareId(t.id);setShowCompare(true);}else{setTheme(t.id);}}} onMouseMove={e=>{if(window.innerWidth>900){const r=e.currentTarget.getBoundingClientRect();const x=((e.clientX-r.left)/r.width-0.5)*18;const y=((e.clientY-r.top)/r.height-0.5)*18;e.currentTarget.style.transform="perspective(500px) rotateY("+x+"deg) rotateX("+(-y)+"deg) scale(1.04)";}}} onMouseLeave={e=>{e.currentTarget.style.transform="";}} className={`kit-card${active?" active":""${compareMode?" compare-hover":""}`}
                  style={{padding:"16px 14px",borderRadius:14,background:bg,border:active?`1.5px solid ${t.ac}aa`:"1.5px solid rgba(255,255,255,0.05)",cursor:"pointer",textAlign:"left",backdropFilter:"blur(8px)",boxShadow:active?`0 14px 44px ${t.ac}30,0 0 0 1px ${t.ac}25`:"0 2px 8px rgba(0,0,0,0.4)",position:"relative"}}>
          ...
          <div className="detail-grid" style={{display:"grid",gridTemplateColumns:"380px 1fr",gap:40,alignItems:"start"}} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
            ...
            <div className="sticky-card" style={{position:"sticky",top:80}}>
              <div key={th.id} className="kit-slide slide-left glass-shimmer" style={{borderRadius:24,...glassStyle,overflow:"hidden",boxShadow:glow?`0 24px 80px rgba(0,0,0,0.6), 0 0 60px ${th.ac}15`:"0 24px 60px rgba(0,0,0,0.5)"}}>
