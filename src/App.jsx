...
function ColorPrism({th,glow}){
  ...
  useState(()=>{
    let id;
    const tick=()=>{setRot(r=>(r+0.4)%360);id=requestAnimationFrame(tick);};
    id=requestAnimationFrame(tick);
    return()=>cancelAnimationFrame(id);
  },[]);
  const s=110;const h=s*0.866;
  const faces=[
    {bg:th.up,  transform:`rotateY(0deg)   translateZ(${s*0.577}px)`},
    {bg:th.dn,  transform:`rotateY(120deg) translateZ(${s*0.577}px)`},
    {bg:th.ac,  transform:`rotateY(240deg) translateZ(${s*0.577}px)`},
    {bg:th.wk,  transform:`rotateY(60deg)  translateZ(${s*0.577}px) rotateX(180deg)`},
    {bg:th.bg,  transform:`rotateX(90deg)  translateZ(${s*0.3}px)`},
    {bg:th.sf,  transform:`rotateX(-90deg) translateZ(${s*0.3}px)`},
  ];
  return(
    <div style={{width:s,height:s,perspective:600,perspectiveOrigin:"50% 50%",margin:"0 auto"}}>
      <div style={{width:s,height:s,position:"relative",transformStyle:"preserve-3d",transform:`rotateX(-18deg) rotateY(${rot}deg)`,transition:"none"}}>
        {faces.map((f,i)=>(
          <div key={i} style={{
            ...
            boxShadow:glow?`inset 0 0 30px ${f.bg}40`:"none",
            opacity:0.92,
          }}/>
        ))}
      </div>
...
function renderKL(T,th,F){const{opens:O,highs:H,lows:L,mids:M}=F;const k=T.kl;return(<div>
  ...
  <SH color={H} label="RESISTANCE — HIGHS"/>
  {[[k.pdh,"Prev Day High","PDH"],[k.pwh,"Prev Week High","PWH"],[k.pmh,"Prev Month High","PMH"],[k.monHigh,"Monday High",""],[k.rthHigh,"RTH High",""],[k.onHigh,"Overnight High","ONH"],[k.srDayR,"Daily Resistance","Fill — zone",false],[k.srWkR,"Weekly Resistance","Fill — zone",false]].map(([h,l,s,ln=true])=><ColorRow key={l} hex={h} label={l} sub={s} th={th} isLine={ln}/>)}
  <SH color={L} label="SUPPORT — LOWS"/>
  {[[k.pdl,"Prev Day Low","PDL"],[k.pwl,"Prev Week Low","PWL"],[k.pml,"Prev Month Low","PML"],[k.monLow,"Monday Low",""],[k.rthLow,"RTH Low",""],[k.onLow,"Overnight Low","ONL"],[k.srDayS,"Daily Support","Fill — zone",false],[k.srWkS,"Weekly Support","Fill — zone",false]].map(([h,l,s,ln=true])=><ColorRow key={l} hex={h} label={l} sub={s} th={th} isLine={ln}/>)}
  ...
  return(
    <div style={{minHeight:"100vh",background:"#05050f",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        {/* Footer */}
        <div style={{borderTop:"1px solid rgba(255,255,255,0.05)",padding:"32px 56px",display:"flex",alignItems:"center",gap:16}}>
