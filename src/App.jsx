        ...
        <div className="main-pad" style={{padding:"72px 52px 48px",maxWidth:1500,margin:"0 auto"}}>
          ...
          {!gridCollapsed&&(
            <div className="hide-desktop" style={{display:"flex",gap:10,marginBottom:14,padding:"12px 14px",borderRadius:14,background:SBGS[th.id]||PHANTOM_SBGS[th.id]||th.bg,border:"1px solid "+th.ac+"55",alignItems:"center"}}>
              ...
              <button onClick={()=>{setGridCollapsed(false);window.scrollTo({top:0,behavior:"smooth"});}} style={{flexShrink:0,padding:"7px 13px",borderRadius:10,background:th.ac+"22",color:th.ac,border:"1px solid "+th.ac+"44",fontSize:10,fontFamily:"'JetBrains Mono',monospace",cursor:"pointer",letterSpacing:"0.06em",whiteSpace:"nowrap",fontWeight:700}}>
                CHANGE ▶
              </button>
            </div>
          )}
          ...
          {gridCollapsed&&<div className="kit-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:9,marginBottom:14}} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
            ...
            {displayed.map(t=>{
              ...
              return(
                <button key={t.id} onClick={()=>{if(compareMode&&t.id!==activeTheme){setCompareId(t.id);setShowCompare(true);}else{setTheme(t.id);if(window.innerWidth<=900){setGridCollapsed(true);setTimeout(()=>document.getElementById('kit-detail')?.scrollIntoView({behavior:'smooth',block:'start'}),50);}}} onMouseMove={e=>{if(window.innerWidth>900){const r=e.currentTarget.getBoundingClientRect();const x=((e.clientX-r.left)/r.width-0.5)*18;const y=((e.clientY-r.top)/r.height-0.5)*18;e.currentTarget.style.transform="perspective(500px) rotateY("+x+"deg) rotateX("+(-y)+"deg) scale(1.04)";}}} onMouseLeave={e=>{e.currentTarget.style.transform="";hideTooltip();}} onMouseEnter={e=>{if(window.innerWidth>900)showTooltip(t,e);}} className={"kit-card"+(active?" active":"")+(compareMode?" compare-hover":"")} style={{'--card-glow':t.ac+'30',padding:"16px 14px",borderRadius:14,background:bg,border:active?"1.5px solid "+t.ac+"aa":"1.5px solid rgba(255,255,255,0.05)",cursor:"pointer",textAlign:"left",backdropFilter:"blur(8px)",boxShadow:active?"0 14px 44px "+t.ac+"30,0 0 0 1px "+t.ac+"25":"0 2px 8px rgba(0,0,0,0.4)",position:"relative"}}>
