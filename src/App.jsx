...
function tools(O,H,L,M,A){return{fib:[{level:"0",hex:L},{level:"0.236",hex:op(L,60)},{level:"0.382",hex:op(L,45)},{level:"0.5",hex:M},{level:"0.618",hex:A},{level:"0.705",hex:op(H,55)},{level:"0.786",hex:op(H,75)},{level:"1",hex:H},{level:"1.618",hex:op(H,82)}],lines:{trend:A,ray:op(A,74),hline:op(A,56),vline:op(A,44)},zones:{support:op(L,28),resistance:op(H,28),bull:op(L,18),bear:op(H,18),fvg:op(M,24),orderBlock:op(A,20),equilibrium:op(M,20),premium:op(H,14),discount:op(L,14)},mas:{ma1:O,ma2:A,ma3:M,ma4:L,ma5:H},channels:{outerBand:A,midline:op(A,60),fill:op(A,9),extended:op(A,44)},vwap:{vwap:O,anchor:M,band1pos:op(H,74),band1neg:op(L,74),band2pos:op(H,54),band2neg:op(L,54),band3pos:op(H,36),band3neg:op(L,36)},pivots:{r3:H,r2:op(H,80),r1:op(H,64),pp:O,s1:op(L,64),s2:op(L,80),s3:L,weekly:op(A,50),monthly:op(A,34)},fractals:{fractalUp:H,fractalDown:L,fractalZone:op(M,18)},volProfile:{poc:O,valueArea:op(M,24),vah:H,val:L,hvn:A,lvn:op(A,38)},indicators:{rsi:A,macd:O,macdSig:M,histBull:L,histBear:H,bbUp:op(H,62),bbDn:op(L,62),bbFill:op(M,9)},ict:{ifvg:op(M,28),breaker:op(H,30),mitigation:op(L,26),rejection:op(H,24),bos:H,choch:M,mss:op(H,74),liqSweep:O,kzLon:op(O,9),kzNY:op(H,7),kzAsia:op(L,7),kzSyd:op(M,7),ote:op(M,24),po3Bull:op(L,28),po3Bear:op(H,28),silverBullet:op(O,14),pdh:H,pdl:L},kl:{
    dOpen:O,wOpen:O,mOpen:O,yOpen:O,rthOpen:O,pdc:op(O,62),
    ...
    srDayH:op(H,30),srDayL:op(L,30),
    srWkH:op(H,22),srWkL:op(L,22),
    srMoH:op(H,15),srMoL:op(L,15),
    dNPOC:op(O,85),wNPOC:op(O,65),mNPOC:op(O,45),
  }}}

// ─── 3D SCENE ────────────────────────────────────────────────────────────────
function ThreeScene({th,glow}){
  const ref=useState(null)[0];
  const mountRef=useState(null);
  const sceneRef=useState({renderer:null,scene:null,mesh:null,frame:null});

  useState(()=>{});

  return useState(()=>{
    // We'll use CSS 3D instead of Three.js for reliability
    return null;
  });
}

// CSS 3D Cube - proper geometry
function ColorPrism({th,glow}){
  const[rot,setRot]=useState(0);
  useEffect(()=>{
    let id;
    const tick=()=>{setRot(r=>(r+0.35)%360);id=requestAnimationFrame(tick);};
    id=requestAnimationFrame(tick);
    return()=>cancelAnimationFrame(id);
  },[]);
  const s=100;const h=s/2;
  // 6 faces of a proper cube
  const faces=[
    {bg:th.up,  t:`translateZ(${h}px)`,                         brightness:1.0},
    {bg:th.dn,  t:`rotateY(180deg) translateZ(${h}px)`,         brightness:0.6},
    {bg:th.ac,  t:`rotateY(90deg) translateZ(${h}px)`,          brightness:0.85},
    {bg:th.wk,  t:`rotateY(-90deg) translateZ(${h}px)`,         brightness:0.7},
    {bg:th.sf,  t:`rotateX(90deg) translateZ(${h}px)`,          brightness:0.9},
    {bg:th.bg,  t:`rotateX(-90deg) translateZ(${h}px)`,         brightness:0.5},
  ];
  return(
    <div style={{width:s,height:s,perspective:700,perspectiveOrigin:"50% 60%",margin:"0 auto"}}>
      <div style={{
        ...
        {faces.map((f,i)=>(
          <div key={i} style={{
            ...
            boxShadow:glow?`inset 0 0 24px ${f.bg}50`:"inset 0 0 12px rgba(0,0,0,0.3)",
          }}/>
        ))}
      </div>
    </div>
...
function Stars(){
  ...
  useEffect(()=>{
    ...
    const stars=Array.from({length:180},()=>({
      ...
      r:Math.random()*1.2+0.2,o:Math.random()*0.6+0.1,speed:Math.random()*0.3+0.05,
    }));
    let frame;
    const draw=()=>{
      ...
      stars.forEach(s=>{
        ...
        ctx.fillStyle=`rgba(255,255,255,${Math.max(0.05,Math.min(0.7,s.o))})`;
        ctx.fill();
      });
      frame=requestAnimationFrame(draw);
    };
    draw();
    return()=>{cancelAnimationFrame(frame);canvas.remove();window.removeEventListener('resize',resize);};
  },[]);
  return null;
}
...
function ChartSwatch({hex,label,glow}){
  ...
  return(
    <div onClick={()=>{navigator.clipboard?.writeText(b);sc(true);setTimeout(()=>sc(false),1400);}}
      style={{display:"flex",flexDirection:"column",gap:6,alignItems:"center",cursor:"pointer",borderRadius:12,padding:"4px",transition:"all 0.15s"}}
      ...
      <div style={{width:"100%",height:36,borderRadius:10,background:hex,
        ...
        border:`2px solid ${cp?"rgba(255,255,255,0.4)":"rgba(255,255,255,0.08)"}`,
        transition:"border 0.15s",position:"relative",overflow:"hidden"}}>
        {cp&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.45)",fontSize:9,fontFamily:"'JetBrains Mono',monospace",color:"#90e890",letterSpacing:"0.1em",fontWeight:700}}>✓</div>}
      </div>
      <span style={{fontSize:9,fontFamily:"'JetBrains Mono',monospace",color:cp?"#90e890":"#333",textAlign:"center",letterSpacing:"0.04em",transition:"color 0.2s"}}>
...
function ColorRow({hex,label,sub,th,isLine=true}){
  ...
  return(
    <div onClick={()=>{navigator.clipboard?.writeText(b);sc(true);setTimeout(()=>sc(false),1400);}}
      ...
      <div style={{flex:1}}>
        <div style={{fontSize:14,color:"#d8d0c8",fontWeight:500,fontFamily:"'Syne',sans-serif",letterSpacing:"0.01em"}}>{label}</div>
        {sub&&<div style={{fontSize:11,color:"#555",marginTop:2,fontFamily:"'JetBrains Mono',monospace"}}>{sub}</div>}
      </div>
      <div style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:cp?"#90e890":"#444",transition:"all 0.2s",letterSpacing:"0.08em",flexShrink:0}}>{cp?"✓ COPIED":b.toUpperCase()}</div>
    </div>
  );
}
...
function SH({color,label}){
  return(
    <div style={{display:"flex",alignItems:"center",gap:10,margin:"24px 0 8px",paddingTop:18,borderTop:"1px solid rgba(255,255,255,0.06)"}}>
      <div style={{width:8,height:8,borderRadius:"50%",background:color,boxShadow:`0 0 14px ${color}`,flexShrink:0}}/>
      <span style={{fontSize:10,letterSpacing:"0.3em",fontFamily:"'JetBrains Mono',monospace",color,fontWeight:700}}>{label}</span>
...
function renderKL(T,th,F){const{opens:O,highs:H,lows:L,mids:M}=F;const k=T.kl;return(<div>
  ...
  {[[k.pdh,"Prev Day High","PDH"],[k.pwh,"Prev Week High","PWH"],[k.pmh,"Prev Month High","PMH"],[k.monHigh,"Monday High",""],[k.rthHigh,"RTH High",""],[k.prevRthH,"Prev RTH High",""],[k.onHigh,"Overnight High","ONH"]].map(([h,l,s])=><ColorRow key={l} hex={h} label={l} sub={s} th={th}/>)}

  <SH color={L} label="PREVIOUS LOWS"/>
  {[[k.pdl,"Prev Day Low","PDL"],[k.pwl,"Prev Week Low","PWL"],[k.pml,"Prev Month Low","PML"],[k.monLow,"Monday Low",""],[k.rthLow,"RTH Low",""],[k.prevRthL,"Prev RTH Low",""],[k.onLow,"Overnight Low","ONL"]].map(([h,l,s])=><ColorRow key={l} hex={h} label={l} sub={s} th={th}/>)}

  <SH color={op(H,50)} label="S/R LEVELS — DAILY / WEEKLY / MONTHLY"/>
  <ColorRow hex={k.srDayH} label="Daily Resistance Level" sub="Daily S/R High" th={th} isLine={false}/>
  <ColorRow hex={k.srDayL} label="Daily Support Level" sub="Daily S/R Low" th={th} isLine={false}/>
  <ColorRow hex={k.srWkH}  label="Weekly Resistance Level" sub="Weekly S/R High" th={th} isLine={false}/>
  <ColorRow hex={k.srWkL}  label="Weekly Support Level" sub="Weekly S/R Low" th={th} isLine={false}/>
  <ColorRow hex={k.srMoH}  label="Monthly Resistance Level" sub="Monthly S/R High" th={th} isLine={false}/>
  <ColorRow hex={k.srMoL}  label="Monthly Support Level" sub="Monthly S/R Low" th={th} isLine={false}/>

  <SH color={O} label="nPOC — NAKED POINT OF CONTROL"/>
  <ColorRow hex={k.dNPOC} label="Daily nPOC" sub="Untested prev day POC — strong magnet" th={th}/>
  <ColorRow hex={k.wNPOC} label="Weekly nPOC" sub="Untested prev week POC" th={th}/>
  <ColorRow hex={k.mNPOC} label="Monthly nPOC" sub="Untested prev month POC" th={th}/>

  <SH color={M} label="MIDPOINTS"/>
  {[[k.monMid,"Monday Mid"],[k.rthMid,"RTH Midpoint"],[k.onMid,"Overnight Mid"],[k.wkMid,"Weekly Mid"],[k.moMid,"Monthly Mid"]].map(([h,l])=><ColorRow key={l} hex={h} label={l} th={th}/>)}
...
function renderVWAP(T,th,F){const{opens:O,highs:H,lows:L}=F;const v=T.vwap;return(<div>
  ...
  {["+1σ","+2σ","+3σ"].map((l,i)=><ColorRow key={l} hex={[v.band1pos,v.band2pos,v.band3pos][i]} label={l+" Band"} th={th}/>)}
  <SH color={L} label="LOWER BANDS"/>
  {["−1σ","−2σ","−3σ"].map((l,i)=><ColorRow key={l} hex={[v.band1neg,v.band2neg,v.band3neg][i]} label={l+" Band"} th={th}/>)}
</div>);}
...
function renderMore(T,th){const p=T.pivots;const v=T.volProfile;const m=T.mas;const i=T.indicators;return(<div>
  <SH color={th.ac} label="PIVOTS"/>
  {[["R3",p.r3],["R2",p.r2],["R1",p.r1],["PP — Pivot Point",p.pp],["S1",p.s1],["S2",p.s2],["S3",p.s3]].map(([l,h])=><ColorRow key={l} hex={h} label={l} th={th}/>)}
  <SH color={th.ac} label="MOVING AVERAGES"/>
  {[["9 EMA",m.ma1],["21 EMA",m.ma2],["50 SMA",m.ma3],["100 SMA",m.ma4],["200 SMA",m.ma5]].map(([l,h])=><ColorRow key={l} hex={h} label={l} th={th}/>)}
  <SH color={th.ac} label="VOLUME PROFILE"/>
  <ColorRow hex={v.poc} label="Point of Control" th={th}/>
  <ColorRow hex={v.vah} label="Value Area High" th={th}/><ColorRow hex={v.val} label="Value Area Low" th={th}/>
  <ColorRow hex={v.valueArea} label="Value Area Fill" th={th} isLine={false}/>
  <SH color={th.ac} label="INDICATORS"/>
  <ColorRow hex={i.rsi} label="RSI" th={th}/><ColorRow hex={i.macd} label="MACD" th={th}/>
  <ColorRow hex={i.macdSig} label="MACD Signal" th={th}/>
  <ColorRow hex={i.histBull} label="Histogram Bull" th={th} isLine={false}/>
  <ColorRow hex={i.histBear} label="Histogram Bear" th={th} isLine={false}/>
  <ColorRow hex={i.bbUp} label="Bollinger Upper" th={th}/><ColorRow hex={i.bbDn} label="Bollinger Lower" th={th}/>
</div>);}
...
const SECS=[
  ...
  {key:"other",label:"More Tools",render:(T,th,F)=>renderMore(T,th)},
];

// Glass style helper
const glass=(accent="transparent",glow=false)=>({
  background:"rgba(255,255,255,0.04)",
  backdropFilter:"blur(20px)",
  WebkitBackdropFilter:"blur(20px)",
  border:`1px solid rgba(255,255,255,0.08)`,
  boxShadow:glow?`0 0 60px ${accent}25, 0 20px 60px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.1)`:"0 20px 60px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.06)",
});
...
export default function App(){
  ...
  const cats=[{id:"ALL",label:"ALL ✦",dot:"#e8b84b"},...Object.entries(CAT_META).map(([id,m])=>({id,label:m.label,dot:m.dot}))];
  ...
  return(
    <div style={{minHeight:"100vh",background:"#05050f",position:"relative",overflow:"hidden"}}>
      ...
      {/* Background ambient glow based on theme */}
      <div style={{position:"fixed",top:"20%",left:"50%",transform:"translateX(-50%)",width:600,height:600,borderRadius:"50%",background:`radial-gradient(circle, ${th.ac}08 0%, transparent 70%)`,pointerEvents:"none",transition:"background 1s ease",zIndex:0}}/>

      {/* Grid lines overlay */}
      <div style={{position:"fixed",inset:0,backgroundImage:"linear-gradient(rgba(255,255,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.02) 1px, transparent 1px)",backgroundSize:"80px 80px",pointerEvents:"none",zIndex:0}}/>

      <div style={{position:"relative",zIndex:1}}>

        {/* ── NAV ── */}
        <nav style={{position:"sticky",top:0,zIndex:100,...glass(),borderRadius:0,borderLeft:"none",borderRight:"none",borderTop:"none",padding:"0 56px",display:"flex",alignItems:"center",height:68,gap:24}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:36,height:36,borderRadius:10,background:`linear-gradient(135deg, ${th.ac}, ${th.ac}88)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,boxShadow:`0 0 30px ${th.ac}50`,transition:"all 0.4s"}}>◈</div>
            <div>
              <div style={{fontSize:16,fontWeight:800,letterSpacing:"-0.02em",fontFamily:"'Syne',sans-serif",lineHeight:1}}>Trading Color Kits</div>
              <div style={{fontSize:9,color:"#333",letterSpacing:"0.25em",fontFamily:"'JetBrains Mono',monospace",marginTop:1}}>TRADINGVIEW COLOR SYSTEM</div>
            </div>
          </div>
          <div style={{flex:1}}/>
          <span style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color:"#333",letterSpacing:"0.15em"}}>90 KITS · 16 CATEGORIES</span>
          <div style={{display:"flex",gap:4,padding:4,borderRadius:24,...glass(),boxShadow:"none"}}>
            {["match","vivid"].map(p=>{const a=palette===p;return(
              <button key={p} onClick={()=>setPal(p)} style={{padding:"7px 18px",borderRadius:20,background:a?th.ac:"transparent",color:a?"#000":"#555",border:"none",fontSize:11,fontFamily:"'JetBrains Mono',monospace",cursor:"pointer",transition:"all 0.2s",fontWeight:a?"700":"400",boxShadow:a&&glow?`0 0 20px ${th.ac}80`:"none"}}>
        ...
        {/* ── HERO ── */}
        <div style={{padding:"80px 56px 60px",maxWidth:1500,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:60,alignItems:"center"}}>
          <div style={{animation:"fadeUp 0.6s ease forwards"}}>
            <div style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color:th.ac,letterSpacing:"0.35em",marginBottom:20,textShadow:glow?`0 0 20px ${th.ac}`:"none"}}>✦ THE DEFINITIVE TRADINGVIEW COLOR REFERENCE</div>
            <h1 style={{fontSize:"clamp(52px,6vw,92px)",fontWeight:800,lineHeight:0.9,letterSpacing:"-0.04em",fontFamily:"'Syne',sans-serif",marginBottom:24}}>
              Chart<br/>
              <span style={{WebkitTextStroke:`1px ${th.ac}`,WebkitTextFillColor:"transparent",textShadow:glow?`0 0 60px ${th.ac}40`:"none",transition:"all 0.4s"}}>Color</span><br/>
              Kits
            </h1>
            <p style={{fontSize:16,color:"#444",maxWidth:480,lineHeight:1.7,fontFamily:"'Syne',sans-serif",marginBottom:40}}>
              90 curated color systems for TradingView. Every kit covers Key Levels, ICT/SMC, Fibonacci, VWAP and more. Click any color to copy.
            </p>
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              <div style={{display:"flex",alignItems:"center",gap:8,padding:"12px 24px",borderRadius:40,...glass(th.ac,glow),cursor:"pointer"}}>
                <div style={{width:8,height:8,borderRadius:"50%",background:th.ac,boxShadow:`0 0 10px ${th.ac}`}}/>
                <span style={{fontSize:12,fontFamily:"'JetBrains Mono',monospace",color:"#888",letterSpacing:"0.12em"}}>{th.name} · {cm.label||"CLASSIC"} · {PAL[palette].label}</span>
              </div>
            </div>
          </div>

          {/* 3D Prism Hero */}
          <div style={{display:"flex",justifyContent:"center",alignItems:"center",animation:"float 6s ease-in-out infinite"}}>
            <div style={{position:"relative"}}>
              <div style={{position:"absolute",inset:-60,borderRadius:"50%",background:`radial-gradient(circle, ${th.ac}18 0%, transparent 70%)`,filter:"blur(20px)",animation:"pulse 3s ease-in-out infinite"}}/>
              <ColorPrism th={th} glow={glow}/>
              {/* Orbit rings */}
              <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%) rotateX(70deg)",width:200,height:200,borderRadius:"50%",border:`1px solid ${th.ac}20`,pointerEvents:"none"}}/>
              <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%) rotateX(70deg) rotateZ(45deg)",width:260,height:260,borderRadius:"50%",border:`1px solid ${th.ac}10`,pointerEvents:"none"}}/>
            </div>
          </div>
        </div>

        {/* ── CATEGORY FILTER ── */}
        <div style={{padding:"0 56px",maxWidth:1500,margin:"0 auto 24px"}}>
          <div className="noscroll" style={{display:"flex",gap:8,paddingBottom:4}}>
            {cats.map(c=>{const a=filter===c.id;return(
              <button key={c.id} onClick={()=>setFilter(c.id)} style={{
                flex:"0 0 auto",padding:"9px 20px",borderRadius:24,
                background:a?c.dot:"rgba(255,255,255,0.04)",
                color:a?"#000":c.dot,
                border:`1px solid ${a?c.dot:c.dot+"30"}`,
                fontSize:11,fontFamily:"'JetBrains Mono',monospace",
                cursor:"pointer",transition:"all 0.2s",fontWeight:a?"700":"400",
                letterSpacing:"0.1em",whiteSpace:"nowrap",
                boxShadow:a?`0 0 20px ${c.dot}40`:"none",
              }}>{c.label}</button>
            );})}
          </div>
        </div>
        ...
        {/* ── THEME GRID ── */}
        <div style={{padding:"0 56px",maxWidth:1500,margin:"0 auto 64px"}}>
            ...
            {filtered.map(t=>{
              ...
              return(
                <button key={t.id} onClick={()=>setTheme(t.id)}
                  onMouseEnter={()=>setHovCard(t.id)} onMouseLeave={()=>setHovCard(null)}
                  style={{
                    padding:"18px 16px",borderRadius:16,
                    background:bg,
                    border:`1.5px solid ${active?t.ac+"aa":"rgba(255,255,255,0.06)"}`,
                    cursor:"pointer",transition:"all 0.25s cubic-bezier(0.34,1.56,0.64,1)",
                    textAlign:"left",
                    transform:active?"translateY(-6px) scale(1.04)":hovCard===t.id?"translateY(-3px) scale(1.02)":"none",
                    boxShadow:active?`0 16px 48px ${t.ac}35, 0 0 0 1px ${t.ac}30`:hovCard===t.id?"0 8px 24px rgba(0,0,0,0.4)":"0 2px 8px rgba(0,0,0,0.4)",
                    backdropFilter:"blur(10px)",
                  }}>
                  <div style={{display:"flex",gap:5,marginBottom:12}}>
                    <div style={{width:18,height:18,borderRadius:5,background:t.up,border:`1px solid rgba(255,255,255,0.15)`,boxShadow:tg?`0 0 12px ${t.up}90`:"none"}}/>
                    <div style={{width:18,height:18,borderRadius:5,background:t.dn,border:`1px solid rgba(255,255,255,0.1)`}}/>
                  </div>
                  <div style={{fontSize:11,fontFamily:"'Syne',sans-serif",color:t.tx,fontWeight:700,letterSpacing:"0.04em",marginBottom:4,opacity:0.9}}>{t.name}</div>
                  {tcm.label&&<div style={{fontSize:8,fontFamily:"'JetBrains Mono',monospace",color:tcm.dot,letterSpacing:"0.12em",opacity:0.8}}>{tcm.label}</div>}
                </button>
              );
            })}
          </div>
        ...
        {/* ── DETAIL PANEL ── */}
        <div style={{padding:"0 56px 100px",maxWidth:1500,margin:"0 auto",display:"grid",gridTemplateColumns:"400px 1fr",gap:40,alignItems:"start"}}>

          {/* LEFT KIT CARD */}
          <div style={{position:"sticky",top:80}}>
            <div key={th.id} style={{borderRadius:28,...glass(th.ac,glow),overflow:"hidden",animation:"fadeUp 0.35s ease forwards",transition:"box-shadow 0.4s"}}>
              {/* Gradient top bar */}
              <div style={{height:3,background:`linear-gradient(90deg, transparent, ${th.ac}, transparent)`,opacity:glow?1:0.4}}/>

              <div style={{padding:"36px 32px 28px"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
                  <span style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:th.ac,letterSpacing:"0.3em",textShadow:glow?`0 0 20px ${th.ac}`:"none"}}>{th.name}</span>
                  {cm.label&&<span style={{fontSize:8,fontFamily:"'JetBrains Mono',monospace",padding:"3px 9px",borderRadius:20,...glass(cm.dot),color:cm.dot,letterSpacing:"0.14em",boxShadow:glow?`0 0 12px ${cm.dot}50`:"none"}}>{cm.label}</span>}
                </div>

                <div style={{fontSize:52,fontWeight:800,lineHeight:0.92,fontFamily:"'Syne',sans-serif",marginBottom:8,letterSpacing:"-0.03em",textShadow:glow?`0 0 48px ${th.ac}30`:"none"}}>
                  <span style={{color:th.tx=="#141414"||th.tx=="#101010"?"#f0f0f0":th.tx}}>Chart<br/>Colors</span>
                </div>
                <div style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color:"#333",letterSpacing:"0.1em",marginBottom:28}}>{th.tag.toUpperCase()}</div>

                {/* 3D prism mini */}
                <div style={{marginBottom:28,display:"flex",justifyContent:"center",padding:"24px",background:"rgba(0,0,0,0.3)",borderRadius:16,border:"1px solid rgba(255,255,255,0.04)"}}>
                  <ColorPrism th={th} glow={glow}/>
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:24}}>
                  {Object.entries(th.chart).map(([k,s])=>(
                    <ChartSwatch key={k} hex={s.hex} label={s.label} glow={glow}/>
                  ))}
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {[["OPENS",fam.opens],["HIGHS",fam.highs],["LOWS",fam.lows],["MIDS",fam.mids]].map(([label,color])=>(
                    <div key={label} style={{display:"flex",alignItems:"center",gap:9,padding:"10px 12px",borderRadius:10,background:`${color}12`,border:`1px solid ${color}25`,transition:"all 0.3s"}}>
                      <div style={{width:10,height:10,borderRadius:"50%",background:color,boxShadow:`0 0 ${glow?"20px":"10px"} ${color}`}}/>
                      <span style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color,fontWeight:700}}>{label}</span>
                    </div>
                  ))}
                </div>
              </div>

              <div style={{borderTop:"1px solid rgba(255,255,255,0.05)",padding:"16px 32px",display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:6,height:6,borderRadius:"50%",background:th.ac,boxShadow:`0 0 10px ${th.ac}`,animation:"pulse 2s ease-in-out infinite"}}/>
                <span style={{fontSize:9,fontFamily:"'JetBrains Mono',monospace",color:"#333",letterSpacing:"0.12em"}}>{PAL[palette].label} · {PAL[palette].desc.toUpperCase()}</span>
              </div>
            </div>
          </div>
          ...
          {/* RIGHT CONTENT */}
          <div>
            <div className="noscroll" style={{display:"flex",gap:6,marginBottom:20}}>
              {SECS.map(s=>{const a=activeTab===s.key;return(
                <button key={s.key} onClick={()=>setTab(s.key)} style={{
                  flex:"0 0 auto",padding:"11px 22px",borderRadius:24,
                  background:a?th.ac:"rgba(255,255,255,0.04)",
                  color:a?"#000":"#555",
                  border:`1px solid ${a?th.ac:"rgba(255,255,255,0.08)"}`,
                  fontSize:12,fontFamily:"'Syne',sans-serif",
                  cursor:"pointer",transition:"all 0.2s",fontWeight:a?"700":"500",
                  letterSpacing:"0.02em",whiteSpace:"nowrap",
                  boxShadow:a&&glow?`0 0 24px ${th.ac}60`:"none",
                }}>{s.label}</button>
              );})}
            </div>
            ...
            <div style={{...glass(),borderRadius:20,overflow:"hidden"}}>
              <div style={{padding:"16px 24px",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:7,height:7,borderRadius:"50%",background:th.ac,boxShadow:`0 0 12px ${th.ac}`,animation:"pulse 2s ease-in-out infinite"}}/>
                <span style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color:"#333",letterSpacing:"0.2em"}}>
                  {SECS.find(s=>s.key===activeTab)?.label.toUpperCase()} · CLICK ROW TO COPY HEX
                </span>
              </div>
              <div style={{padding:"8px 8px 20px",maxHeight:660,overflowY:"auto"}}>
                {sec?.render(T,th,fam)}
              </div>
            </div>

            <div style={{marginTop:14,padding:"16px 24px",...glass(),borderRadius:16,display:"flex",gap:14,alignItems:"center"}}>
              <span style={{fontSize:18}}>💡</span>
              <span style={{fontSize:13,fontFamily:"'Syne',sans-serif",color:"#444",lineHeight:1.5}}>
                Copy hex → TradingView → right-click any tool → Edit → paste color
              </span>
            </div>
          </div>
        </div>
        ...
        {/* Footer */}
        <div style={{borderTop:"1px solid rgba(255,255,255,0.05)",padding:"28px 56px",display:"flex",alignItems:"center",gap:16}}>
          <div style={{width:24,height:24,borderRadius:6,background:`linear-gradient(135deg, ${th.ac}, ${th.ac}55)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12}}>◈</div>
          <span style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:"#222",letterSpacing:"0.15em"}}>TRADING COLOR KITS · 90 THEMES · 16 CATEGORIES</span>
          <div style={{flex:1}}/>
          <a href="https://x.com/_a_Sian_" target="_blank" rel="noopener noreferrer"
            ...
            <span style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:"#444",letterSpacing:"0.1em"}}>made by</span>
            <span style={{fontSize:11,fontFamily:"'Syne',sans-serif",color:"#666",fontWeight:700,letterSpacing:"0.05em"}}>aSian</span>
          </a>
        </div>
