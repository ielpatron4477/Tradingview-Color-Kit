...
const NEW_KITS_RELEASE = {
  chalk:1745971200000, lead:1745971200000, bone:1745971200000,
  soot:1745971200000, flint:1745971200000, studio:1745971200000,
  darkroom:1745971200000, silveraura:1745971200000, roseaura:1745971200000,
  fairy:1745971200000, voidrift:1745971200000,
  phantom:1745884800000, oracle:1745884800000, specter:1745884800000,
  doge:1745884800000, wif:1745884800000, bonk:1745884800000,
  neonnoir:1745884800000, chromepunk:1745884800000,
  purpleaura:1745884800000, goldenaura:1745884800000,
  acid:1745884800000, glitch:1745884800000, mortis:1745884800000,
  eclipse:1745884800000, canvas:1745884800000, spectrum:1745884800000,
  dragon:1745884800000, elven:1745884800000,
};
...
// ─── LIVE ANIMATED CHART ─────────────────────────────────────────────────────
function LiveChart({th,fam,siteMode,candleStyle='candles'}){
  const canvasRef=useRef(null);
  const stateRef=useRef({candles:[],live:{o:0,h:0,l:0,c:0},tick:0,price:100});
  ...
  useEffect(()=>{
    ...
    const candles=[];
    for(let i=0;i<28;i++){
      const o=price,mov=(Math.random()-0.46)*2.2;
      const c=o+mov,h=Math.max(o,c)+Math.random()*1.2,l=Math.min(o,c)-Math.random()*1.2;
      candles.push({o,h,l,c});price=c;
    }
    stateRef.current.candles=candles;
    stateRef.current.price=price;
    stateRef.current.live={o:price,h:price,l:price,c:price};
    ...
    const draw=()=>{
      ...
      const drift=Math.sin(tickCount*0.03)*0.15;
      s.live.c=Math.max(s.live.o-6,Math.min(s.live.o+6,s.live.c+noise+drift*0.1));
      s.live.h=Math.max(s.live.h,s.live.c+Math.random()*0.3);
      s.live.l=Math.min(s.live.l,s.live.c-Math.random()*0.3);

      // New candle every 180 ticks
      if(tickCount%180===0){
        s.candles.push({...s.live});
        if(s.candles.length>28)s.candles.shift();
        const newO=s.live.c;
        s.live={o:newO,h:newO,l:newO,c:newO};
      }
      ...
      const allCandles=[...s.candles,s.live];
      const prices=allCandles.flatMap(c=>[c.h,c.l]);
      const minP=Math.min(...prices)-1,maxP=Math.max(...prices)+1,range=maxP-minP;
      const py=p=>H-((p-minP)/range)*H*0.88-H*0.06;

      const cw=Math.floor(W/allCandles.length);
      const bw=Math.max(2,cw-3);

      // Grid lines
      ctx.strokeStyle=siteMode==='light'?'rgba(0,0,0,0.06)':'rgba(255,255,255,0.04)';
      ctx.lineWidth=0.5;
      for(let i=1;i<4;i++){const y=H*0.06+H*0.88*(i/4);ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W,y);ctx.stroke();}

      // Key levels
      const midP=(Math.max(...prices)+Math.min(...prices))/2;
      [[fam.opens,midP+range*0.15,"dOpen"],[fam.highs,maxP-range*0.1,"PDH"],[fam.lows,minP+range*0.1,"PDL"]].forEach(([col,p,label])=>{
        ctx.strokeStyle=col;ctx.lineWidth=1;ctx.setLineDash([4,3]);
        ctx.beginPath();ctx.moveTo(0,py(p));ctx.lineTo(W,py(p));ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle=col;ctx.font=`9px monospace`;ctx.fillText(label,W-30,py(p)-2);
      });

      // Compute HA candles if needed
      let drawCandles=allCandles;
      const cs=styleRef.current;
      if(cs==='ha'){
        let haO=allCandles[0].o,haC=allCandles[0].c;
        drawCandles=allCandles.map(c=>{
          const nc=(c.o+c.h+c.l+c.c)/4;
          const no=(haO+haC)/2;
          haO=no;haC=nc;
          return{o:no,h:Math.max(c.h,no,nc),l:Math.min(c.l,no,nc),c:nc};
        });
      }

      // Candles
      drawCandles.forEach((c,i)=>{
        const x=i*cw+1;const isLive=i===drawCandles.length-1;
        const bull=c.c>=c.o;
        const fill=bull?th.up:th.dn;
        const mid=(cw-bw)/2;
        if(isLive){ctx.shadowColor=fill;ctx.shadowBlur=8;}

        if(cs==='bars'){
          ctx.strokeStyle=fill;ctx.lineWidth=1.5;
          ctx.beginPath();ctx.moveTo(x+mid+bw/2,py(c.h));ctx.lineTo(x+mid+bw/2,py(c.l));ctx.stroke();
          ctx.beginPath();ctx.moveTo(x+mid,py(c.o));ctx.lineTo(x+mid+bw/2,py(c.o));ctx.stroke();
          ctx.beginPath();ctx.moveTo(x+mid+bw/2,py(c.c));ctx.lineTo(x+mid+bw,py(c.c));ctx.stroke();
        } else {
          ctx.strokeStyle=th.wk;ctx.lineWidth=1;
          ctx.beginPath();ctx.moveTo(x+mid+bw/2,py(c.h));ctx.lineTo(x+mid+bw/2,py(c.l));ctx.stroke();
          const top=py(Math.max(c.o,c.c)),bot=py(Math.min(c.o,c.c)),bodyH=Math.max(1,bot-top);
          if(cs==='hollow'&&bull){
            ctx.strokeStyle=fill;ctx.lineWidth=1.5;ctx.strokeRect(x+mid,top,bw,bodyH);
          } else {
            ctx.fillStyle=fill;ctx.fillRect(x+mid,top,bw,bodyH);
          }
        }
        ctx.shadowBlur=0;
      });
  ...
  },[th.id,siteMode]);  // eslint-disable-line
  return(
    <div style={{borderRadius:12,overflow:"hidden",background:th.bg,border:`1px solid rgba(255,255,255,0.06)`,position:"relative",cursor:"pointer"}}>
      ...
      <div style={{position:"absolute",top:8,right:8,display:"flex",alignItems:"center",gap:4}}>
        ...
        <span style={{fontSize:8,fontFamily:"'JetBrains Mono',monospace",color:"#20c87080",letterSpacing:"0.1em"}}>LIVE</span>
      </div>
      <div style={{padding:"5px 10px",fontSize:8,fontFamily:"'JetBrains Mono',monospace",color:"#2a2a2a",letterSpacing:"0.1em",background:"rgba(0,0,0,0.2)"}}>ANIMATED PREVIEW · {th.name}</div>
    </div>
  );
}

// ─── FULLSCREEN CHART MODAL ───────────────────────────────────────────────────
function FullscreenChart({th,fam,onClose}){
  const canvasRef=useRef(null);
  ...
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      allC.forEach((c,i)=>{
        const x=padL+i*(cW/allC.length);const bull=c.c>=c.o;
        const fill=bull?th.up:th.dn;const isLive=i===allC.length-1;
        if(isLive){ctx.shadowColor=fill;ctx.shadowBlur=12;}
        ctx.strokeStyle=th.wk;ctx.lineWidth=1;
        ctx.beginPath();ctx.moveTo(x+bw2/2,py(c.h));ctx.lineTo(x+bw2/2,py(c.l));ctx.stroke();
        const top=py(Math.max(c.o,c.c)),bodyH=Math.max(1,py(Math.min(c.o,c.c))-top);
        ctx.fillStyle=fill;ctx.fillRect(x,top,bw2,bodyH);
        ctx.shadowBlur=0;
      });
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:999,background:"rgba(0,0,0,0.94)",display:"flex",flexDirection:"column",animation:"fadeUp 0.3s ease"}} onClick={onClose}>
      <div style={{display:"flex",alignItems:"center",gap:16,padding:"14px 24px",borderBottom:"1px solid rgba(255,255,255,0.06)"}} onClick={e=>e.stopPropagation()}>
        ...
        <span style={{fontSize:12,fontFamily:"'Syne',sans-serif",fontWeight:700,color:"#555",letterSpacing:"0.1em",flex:1}}>{th.name.toUpperCase()} · LIVE CHART · {th.tag.toUpperCase()}</span>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <span style={{fontSize:11,fontFamily:"'JetBrains Mono',monospace",color:"#333"}}>ZOOM</span>
          {[0.5,1,1.5,2].map(z=>(
...
const CHANGELOG = [
  { version:"2.0", date:"Apr 2026", items:[
    "🏆 99 kits total — 11 new added (CHALK, LEAD, BONE, SOOT, FLINT, STUDIO, DARKROOM, SILVER AURA, ROSE AURA, FAIRY, VOID RIFT)",
      ...
      <div style={{background:"#0a0a12",borderRadius:24,padding:28,border:"1px solid rgba(255,255,255,0.08)",maxWidth:480,width:"100%",animation:"fadeUp 0.3s ease",maxHeight:"80vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <div>
            ...
            <div style={{fontSize:22,fontWeight:800,fontFamily:"'Syne',sans-serif",color:"#f0f0f0"}}>Version {CHANGELOG_VERSION}</div>
          </div>
          ...
          <div key={v.version} style={{marginTop:20}}>
            ...
            {v.items.map((item,i)=>(
              <div key={i} style={{display:"flex",gap:10,padding:"7px 0",borderBottom:"1px solid rgba(255,255,255,0.03)"}}>
                <span style={{fontSize:13,flexShrink:0}}>{item.slice(0,2)}</span>
                <span style={{fontSize:12,fontFamily:"'Syne',sans-serif",color:"#555",lineHeight:1.5}}>{item.slice(3)}</span>
              </div>
            ))}
          </div>
        ))}
        <button onClick={onClose} style={{marginTop:20,width:"100%",padding:"12px",borderRadius:12,background:ac,color:"#000",border:"none",fontSize:12,fontFamily:"'Syne',sans-serif",fontWeight:700,cursor:"pointer",letterSpacing:"0.06em"}}>
          Let's go ✦
        </button>
      </div>
    </div>
...
export default function App(){
  ...
  return(
    <div style={{minHeight:"100vh",background:site.bg,position:"relative",transition:"background 0.3s"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <nav style={{position:"sticky",top:0,zIndex:100,...glassStyle,borderRadius:0,borderLeft:"none",borderRight:"none",borderTop:"none"}}>
          <div className="nav-inner" style={{padding:"0 52px",display:"flex",alignItems:"center",height:64,gap:16,maxWidth:1500,margin:"0 auto"}}>
            ...
            <button onClick={()=>setSiteMode(m=>m==='dark'?'light':'dark')} style={{padding:"7px 14px",borderRadius:20,background:site.muted,border:`1px solid ${site.panelBorder}`,color:site.subText,fontSize:12,cursor:"pointer",transition:"all 0.2s",fontFamily:"'JetBrains Mono',monospace",letterSpacing:"0.06em"}} title="Toggle light/dark mode">
                  ...
                  <div style={{marginBottom:16}}>
                    ...
                    {chartMode&&(
                      <div>
                        ...
                        {/* Chart content */}
                        {chartMode==='static'&&<MockChart th={th} fam={fam} candleStyle={candleStyle}/>}
                        {chartMode==='live'&&(
                          <div style={{position:"relative"}}>
                            <LiveChart th={th} fam={fam} siteMode={siteMode} candleStyle={candleStyle}/>
                ...
                {getSimilar(th,palette,4).map(t=>{
                  ...
                  return(
                    <button key={t.id} onClick={()=>setTheme(t.id)} className="kit-card" style={{padding:"14px 12px",borderRadius:12,background:bg,border:"1px solid rgba(255,255,255,0.06)",cursor:"pointer",textAlign:"left",backdropFilter:"blur(8px)"}}>
                      ...
                      <div style={{fontSize:10,fontFamily:"'Syne',sans-serif",color:t.tx,fontWeight:700,marginBottom:6}}>{t.name}</div>
      ...
      {showFullChart&&<FullscreenChart th={th} fam={fam} onClose={()=>setShowFullChart(false)}/>}
