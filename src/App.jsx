...
function ColorRow({hex,label,sub,isLine=true}){
  ...
  return(
    <div onClick={(ev)=>{
        navigator.clipboard?.writeText(b);sc(true);setTimeout(()=>sc(false),1400);
    ...
    <div style={{marginTop:12}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
        {pairs.map((p,i)=>{
          ...
          return(
            <div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 8px",borderRadius:8,background:`${b.color}0c`,border:`1px solid ${b.color}28`}}>
...
export default function App(){
  ...
  const[gridCollapsed,setGridCollapsed]=useState(()=>window.innerWidth<=900);
  // Auto-expand grid on desktop
  useEffect(()=>{
    const check=()=>{if(window.innerWidth>900)setGridExpanded(true);};
  ...
  // Persist last viewed kit
  useEffect(()=>{try{localStorage.setItem('tck-last-kit',activeTheme);}catch{}},[activeTheme]);
        ...
        <div className="main-pad" style={{padding:"72px 52px 48px",maxWidth:1500,margin:"0 auto"}}>
          <div className="hero-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:60,alignItems:"center",marginBottom:64}}>
            <div className="fade-up">
              ...
              <p style={{fontSize:15,color:"#3a3a3a",maxWidth:460,lineHeight:1.7,fontFamily:"'Space Grotesk',sans-serif",marginBottom:32}}>90 curated color systems for TradingView. Key Levels, ICT/SMC, Fibonacci, VWAP, nPOC and more. Click any color to copy.</p>
              {/* Trending */}
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
              {(()=>{const kt=THEMES.find(t=>t.id===kotd);if(!kt)return null;return(
                <button onClick={()=>setTheme(kotd)} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderRadius:12,background:"rgba(255,215,0,0.06)",border:"1px solid rgba(255,215,0,0.18)",cursor:"pointer"}}>
                  ...
                  <div><div style={{fontSize:7,fontFamily:"'JetBrains Mono',monospace",color:"#665500",letterSpacing:"0.2em"}}>KIT OF THE DAY</div><div style={{fontSize:10,fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,color:"#aa8800"}}>{kt.name}</div></div>
                </button>
              );})()}
                ...
                <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                  {recent.filter(id=>id!==activeTheme).slice(0,5).map(id=>{const t=THEMES.find(x=>x.id===id);if(!t)return null;return(
                    <button key={id} onClick={()=>setTheme(id)} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 9px",borderRadius:16,background:SBGS[id]||t.bg,border:`1px solid ${t.ac}40`,cursor:"pointer"}}>
                      <div style={{width:8,height:8,borderRadius:2,background:t.up,flexShrink:0}}/>
                      <span style={{fontSize:8,fontFamily:"'Space Grotesk',sans-serif",color:t.tx,fontWeight:700}}>{t.name}</span>
                    </button>
                  );})}
                </div>
              </div>
          ...
          <div className="show-mobile-only" style={{marginBottom:12}}>
            <button onClick={()=>setGridCollapsed(v=>!v)} style={{width:"100%",padding:"12px 16px",borderRadius:14,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",color:"#888",fontSize:12,fontFamily:"'JetBrains Mono',monospace",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",letterSpacing:"0.1em"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{display:"flex",gap:4}}>
                  {[th.up,th.dn,th.ac].map((col,i)=><div key={i} style={{width:10,height:10,borderRadius:2,background:col}}/>)}
                </div>
                <span style={{color:"#c0b8b0",fontWeight:600}}>{th.name}</span>
              </div>
              <span style={{fontSize:10,color:"#3a3a3a"}}>{gridCollapsed?displayed.length+" KITS ↓":"CLOSE ↑"}</span>
            </button>
          </div>
          {/* THEME GRID */}

          {/* Mobile collapse toggle — hidden on desktop via CSS */}
          <button onClick={()=>setGridExpanded(v=>!v)} className="hide-desktop"
            style={{width:"100%",marginBottom:10,padding:"12px 16px",borderRadius:14,background:gridCollapsed?"rgba(255,255,255,0.06)":"rgba(255,255,255,0.03)",border:"1px solid "+(gridCollapsed?"rgba(255,255,255,0.1)":"rgba(255,255,255,0.07)"),color:"#888",fontSize:11,fontFamily:"'JetBrains Mono',monospace",cursor:"pointer",letterSpacing:"0.1em",display:"flex",alignItems:"center",justifyContent:"space-between",transition:"all 0.2s"}}>
            <span style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:10,display:"inline-block",transform:gridCollapsed?"rotate(90deg)":"rotate(0deg)",transition:"transform 0.2s"}}>▶</span>
              <span>BROWSE ALL KITS</span>
              <span style={{padding:"2px 8px",borderRadius:8,background:"rgba(255,255,255,0.07)",fontSize:9,color:"#555"}}>{displayed.length}</span>
            </span>
            <span style={{fontSize:10,color:"#333"}}>{gridCollapsed?"hide":"tap to browse"}</span>
          </button>

          {/* Active kit mini card — mobile only, shown when grid is collapsed */}
          {!gridCollapsed&&(
            <div className="hide-desktop" style={{display:"flex",gap:10,marginBottom:14,padding:"12px 14px",borderRadius:14,background:SBGS[th.id]||PHANTOM_SBGS[th.id]||th.bg,border:"1px solid "+th.ac+"55",alignItems:"center"}}>
              <div style={{display:"flex",gap:3,flexShrink:0}}>
                {[{fill:th.up,h:20},{fill:th.dn,h:14},{fill:th.up,h:24},{fill:th.up,h:17},{fill:th.dn,h:19}].map((cv,ci)=>(
                  <div key={ci} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
                    ...
                    <div style={{width:7,height:cv.h,borderRadius:2,background:cv.fill}}/>
                    <div style={{width:2,height:2,background:th.wk,borderRadius:1}}/>
                  </div>
                ))}
              </div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,fontWeight:700,color:th.ac,fontFamily:"'Space Grotesk',sans-serif"}}>{th.name}</div>
                <div style={{fontSize:9,color:"#444",fontFamily:"'JetBrains Mono',monospace",letterSpacing:"0.1em",marginTop:2}}>{CAT_META[th.cat]?.label||"CLASSIC"}</div>
              </div>
              <button onClick={()=>{setGridExpanded(true);window.scrollTo({top:0,behavior:"smooth"});}} style={{flexShrink:0,padding:"7px 13px",borderRadius:10,background:th.ac+"22",color:th.ac,border:"1px solid "+th.ac+"44",fontSize:10,fontFamily:"'JetBrains Mono',monospace",cursor:"pointer",letterSpacing:"0.06em",whiteSpace:"nowrap",fontWeight:700}}>
                CHANGE ▶
              </button>
            </div>
          )}
          ...
          {gridCollapsed&&<div className="kit-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:9,marginBottom:14}} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
            ...
            {displayed.map(t=>{
              ...
              return(
                <button key={t.id}
                  onClick={()=>{if(compareMode&&t.id!==activeTheme){setCompareId(t.id);setShowCompare(true);}else{setTheme(t.id);if(window.innerWidth<=900){setGridExpanded(false);setTimeout(()=>document.getElementById('kit-detail')?.scrollIntoView({behavior:'smooth',block:'start'}),50);}}}
                  onMouseMove={e=>{if(window.innerWidth>900){const r=e.currentTarget.getBoundingClientRect();const x=((e.clientX-r.left)/r.width-0.5)*18;const y=((e.clientY-r.top)/r.height-0.5)*18;e.currentTarget.style.transform="perspective(500px) rotateY("+x+"deg) rotateX("+(-y)+"deg) scale(1.04)";}}}
                  onMouseLeave={e=>{e.currentTarget.style.transform="";hideTooltip();}}
                  onMouseEnter={e=>{if(window.innerWidth>900)showTooltip(t,e);}}
                  className={"kit-card"+(active?" active":"")+(compareMode?" compare-hover":"")}
                  style={{'--card-glow':t.ac+'30',padding:"16px 14px",borderRadius:14,background:bg,border:active?"1.5px solid "+t.ac+"aa":"1.5px solid rgba(255,255,255,0.05)",cursor:"pointer",textAlign:"left",backdropFilter:"blur(8px)",boxShadow:active?"0 14px 44px "+t.ac+"30,0 0 0 1px "+t.ac+"25":"0 2px 8px rgba(0,0,0,0.4)",position:"relative"}}>
                  ...
                  <div style={{display:"flex",gap:3,marginBottom:10,alignItems:"flex-end"}}>
                    {[{fill:t.up,h:20},{fill:t.dn,h:14},{fill:t.up,h:24},{fill:t.up,h:17},{fill:t.dn,h:19}].map((cv,ci)=>(<div key={ci} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1}}><div style={{width:2,height:3,background:t.wk,borderRadius:1}}/><div style={{width:7,height:cv.h,borderRadius:2,background:cv.fill,boxShadow:tg?"0 0 6px "+cv.fill+"80":"none"}}/><div style={{width:2,height:2,background:t.wk,borderRadius:1}}/></div>))}
                  </div>
                  <div style={{fontSize:10,fontFamily:"'Space Grotesk',sans-serif",color:active?t.tx:"rgba(255,255,255,0.5)",fontWeight:700,letterSpacing:"0.04em",marginBottom:3}}><Highlight text={t.name} query={search} color={t.ac}/></div>
